 ...
 */
public class FxAutoCompleteTextField extends TextField {
	 ...
	 */
	public FxAutoCompleteTextField(SortedSet<String> entrySet) {
		super();
		...
		textProperty().addListener(new ChangeListener<String>() {
			...
			public void changed(ObservableValue<? extends String> observableValue, String s, String s2) {
				if (getText().length() == 0) {
					...
					String text = getText();
					Pattern pattern;
					// * . gelirse patlıyor özel karakterlere önlemek almak lazım
					if (isCaseSensitive()) {
					...
					if (entrySet.size() > 0) {
						...
						if (!isPopupHidden()) {
							...
							if (!entriesPopup.isShowing()) {
								entriesPopup.show(FxAutoCompleteTextField.this, Side.BOTTOM, 0, 0);
		...
		// with enter key event, it selects the first item of filtered entries (if exists)
		addEventHandler(KeyEvent.KEY_RELEASED, new EventHandler<KeyEvent>() {
			...
			public void handle(KeyEvent event) {
				...
				if (event.getCode() == KeyCode.ENTER) {
					...
					//Loghelperr.getInstance(getClass()).debug("enter autocomp2");
					if (getFilteredEntries().size() > 0) {
						setText(getFilteredEntries().get(0));
						// to move caret position to the last
		...
		focusedProperty().addListener(new ChangeListener<Boolean>() {
			...
			public void changed(ObservableValue<? extends Boolean> observableValue, Boolean aBoolean, Boolean aBoolean2) {
				//Loghelperr.getInstance(getClass()).debug("focused Prop Changed");
				entriesPopup.hide();
			}
		...
		//onKeyReleasedProperty().addListener();
