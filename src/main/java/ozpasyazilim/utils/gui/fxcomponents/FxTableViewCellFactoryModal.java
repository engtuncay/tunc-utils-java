...
public class FxTableViewCellFactoryModal {
  ...
  public static <EntClazz> void setupCellFactoryGeneral(FxTableCol2 fxTableCol, Class<EntClazz> entitClazz) {
    ...
    //Loghelper.get(getClassi()).debug("setupCellFactoryGeneral called");
    //Loghelper.get(getClassi()).debug("FieldName:" + fxTableCol.getRefFiCol().getFcTxFieldName());

    //String txClassName = FxEditorFactory.calcTxClassNameMain2(fxTableCol.getRefFiCol(), fxTableCol.getRefFiCol().getColType());

    String txColEditorClass = fxTableCol.getRefFiCol().getColEditorClass();

    // Editor Class belirtilmişse , Cell Factory'si editorClass a göre oluşturulur.
    if (!FiString.isEmpty(txColEditorClass)) {
      ...
      if (txColEditorClass.equals(Button.class.getSimpleName())
          || txColEditorClass.equals(Button.class.getName())) {
      ...
      if (txColEditorClass.equals(Node.class.getName())) {
      ...
      if (txColEditorClass.equals(EnumColNodeType.FxButtonCustom.toString())) {
      ...
      if (txColEditorClass.equals(EnumColNodeType.FxLabelStatus.toString())) {
        fxTableCol.styleAlignCenterFi();
    ...
    FiCol refFiCol = fxTableCol.getRefFiCol();

    FxEditorFactory.assignFieldTypeByOzColType(refFiCol);

    String txFieldType = refFiCol.getFcTxFieldType();

    if (!FiString.isEmpty(txFieldType)) {

      if (FimFtFieldTypeSpec.isDouble(txFieldType)) {
        assignCellFactoryLabelForDoubleTypes(fxTableCol);
        return;
      }

      if (FimFtFieldTypeSpec.isInteger(txFieldType)) {
        assignStyleToFxColForRightAlign(fxTableCol);
        return;
      }

      if (FimFtFieldTypeSpec.isDate(txFieldType)) {
        assignCellFactoryDateType(fxTableCol);
        return;
      }

      if (FimFtFieldTypeSpec.isBool(txFieldType)) {
...
//			} );
  ...
  private static void assignStyleToFxColForRightAlign(FxTableCol2 fxTableCol) {
    /*
  ...
  private static <EntClazz> void assignCellFactoryDateType(FxTableCol2 fxTableCol) {
    ...
    fxTableCol.setCellFactory(new CellFactoryLabelWitFormat<EntClazz, Date>(f));
  }

  private static <EntClazz> void assignCellFactoryLabelForDoubleTypes(FxTableCol2 fxTableCol) {
    ...
    fxTableCol.setCellFactory(new CellFactoryLabelWitFormat<EntClazz, Double>(decimalFormatter));
    assignStyleToFxColForRightAlign(fxTableCol);
  }
...
//											}
  ...
  private static Callback<TableColumn, TableCell> getCellFactoryForCheckBoxSimple() {
    // anonymous class or lambda function Callback function input TableColumn,output TableCell
//		return new Callback<TableColumn, TableCell>() {
//			@Override
...
// Loghelper.get(FxTableViewCellEditorFactoryConfig.class).debug("Cell Editor Setup Col Name:" + fxTableCol.getFiCol().getFieldName());
    ...
    if (dataType == OzColType.Date || (dataType == null && classNameSimple.equals("Date"))) {
      SimpleDateFormat f = new SimpleDateFormat("dd.MM.yy");
      fxTableCol.setCellFactory(new CellFactoryLabelWitFormat<EntityClass, Date>(f));
    }
