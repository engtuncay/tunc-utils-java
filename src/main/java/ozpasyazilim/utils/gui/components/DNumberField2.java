...
public class DNumberField2 extends JTextField {
	...
	public DNumberField2(Integer digitNum, Integer decimalNum, String npb) {
		super();
	...
	public boolean isValid(String ns) {
		...
		if (nch.length > 0) {
			if (nch[0] == '-' && npb.equals("p")) {
				OzPanel.getMessagePane(this).showInfoPane(
					"Bu alana sadece pozitif sayı girebilirsiniz");
				return false;
			} else if (nch[0] != '-' && npb.equals("n")) {
				OzPanel.getMessagePane(this).showInfoPane(
					"Bu alana sadece negatif sayı girebilirsiniz");
				return false;
		...
		if (ns.length() > maxL) {
			OzPanel.getMessagePane(this).showInfoPane("Bu alana daha fazla girdi yapamazs�n�z");
			return false;
		}
		...
		for (; digit < ns.length(); digit++) {
			if (nch[digit] < '0' || nch[digit] > '9') {
				if (nch[digit] == ',') {
					if (decimalNum == 0) {
						OzPanel.getMessagePane(this).showInfoPane(
							"Bu alana sadece Tamsay� girilebilir");
						return false;
					} else
						break;
				}
				OzPanel.getMessagePane(this).showInfoPane(
					"Bu alana sadece say�sal de�er girebilirsiniz :" + nch[digit]);
				return false;
			}
		}
		...
		if (digit > digNum) {
			if (decimalNum == 0) {
				OzPanel.getMessagePane(this).showInfoPane("Bu alana en fazla "
					+ digitNum
					+ " basamakl� tamsay� girebilirsiniz");
			} else
				OzPanel.getMessagePane(this).showInfoPane("Bu alan�n tamsay� k�sm� enfazla "
					+ digitNum
					+ " basamakl� olabilir");
			return false;
		}
		...
		for (; digit < ns.length(); digit++) {
			...
			if (nch[digit] < '0' || nch[digit] > '9') {
				OzPanel.getMessagePane(this).showInfoPane(
					"Bu alana sadece say�sal de�er girebilirsiniz (" + nch[digit] + " girilemez)");
				return false;
			}
		}

		if (rate > decimalNum) {
			OzPanel.getMessagePane(this).showInfoPane("Bu alan�n ondal�k k�sm� en fazla "
				+ decimalNum
				+ " basamak olabilir");
			return false;
		}
	...
	class NumberPlainDocument extends PlainDocument {
		private String myInsert(String num, int offs, String str) {
			//        System.out.println("num="+num+"l:"+num.length()+" offs= "+offs+" str="+str);
			if (num.length() == offs) {
		...
		public void replace(int offs, int len, String text, AttributeSet attrs)
			...
			String num = super.getText(0, super.getLength());
			if (isValid(myInsert(myRemove(offs, len, num), offs, text))) super.replace(offs, len,
				text, attrs);
		}
