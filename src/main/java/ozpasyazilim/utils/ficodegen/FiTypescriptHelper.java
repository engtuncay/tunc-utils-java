...
public class FiTypescriptHelper {
	...
	public static String tsEntity(Class<?> clazz) {
		...
		List<FiField> listFiFieldsSummary = FiReflectClass.getListFieldsWoutStatic(clazz, true);
		...
		for (FiField fiField : listFiFieldsSummary) {
			...
			result.append(String.format("\t%s: %s;",fiField.getFcTxFieldName(),typeTypesript));
