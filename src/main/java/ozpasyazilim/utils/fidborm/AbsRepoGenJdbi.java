 ...
 * @param <EntClazz>
 */
public abstract class AbsRepoGenJdbi<EntClazz> extends AbsRepoJdbi implements IRepoJdbi {
    ...
    public Fdr<List<EntClazz>> jdSelectAllDtoOrderByCandId() {
        ...
        try {
            result = getJdbi().withHandle(handle -> {
                return handle.select(FiQueryGenerator.selectDtoFieldsWoutWhereOrderByCandIdField(getEntityClass()))
                        .mapToBean(getEntityClass())
                        .list();
            });
            fdr.setBoResult(true);
...
//		Loghelper.get(getClass()).debug(FiConsole.logMain(fiMapParams));
    ...
    public Fdr<List<EntClazz>> jdSelectListAllDtoByFiWhere1(EntClazz entClazz) {
        String sql = FiQueryGenerator.selectAllDtoByFiWhere1(getEntityClass());
    ...
    public Fdr jdhInsertEntityDbNameAdded(Handle handle, EntClazz entity) {
        ...
        try {
            rowCountUpdate = handle.createUpdate(FiQueryGenerator.insertQueryRtDbName(getEntityClass(), this))
                    .bindBean(entity)
                    .execute();
            ...
            jdbi.useTransaction(handle -> {
                ...
                try {
                    ...
                    listEntity.forEach(ent -> {
                        ...
                        int execute = handle.createUpdate(FiQueryGenerator.insertQueryWithId(getEntityClass()))
                                .bindBean(ent)
                                .execute();// returns row count updated
        ...
        try {
            boResult = jdbi.inTransaction(handle -> {
                ...
                try {
                    // transactions
                    listEntity.forEach(ent -> {
                        handle.createUpdate(FiQueryGenerator.insertQueryJParamWoutId(getEntityClass()))
                                .bindBean(ent)
                                .execute(); // returns row count updated
...
//		fdr.setValue(0);
    ...
    public Fdr<EntClazz> jdSelectEntityBindMap(String sql, Map<String, Object> mapParam) {
        ...
        try {
            ...
            Loghelper.get(getClass()).error("Query Problem");
            Loghelper.get(getClass()).debug(FiException.exceptionToStrMain(ex));
            fdr.setBoResult(false, ex);
        }
