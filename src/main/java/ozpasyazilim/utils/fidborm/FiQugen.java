...
//				--     (SELECT MAX(no)+1  FROM PERSON), @name
  ...
  public static String updateFiColsByBoWhereFields(List<FiCol> listFields, IFiTableMeta iFiTableMeta) {

    String template = "UPDATE {{tableName}} SET {{setParams}} \n"
        + " WHERE {{whereCond}} ";

    StringBuilder sbSetParams = new StringBuilder();
    StringBuilder sbWhereCond = new StringBuilder();

    int indexSetParams = 1;
    int indexWhereParams = 1;

    for (FiCol fiCol : listFields) {

      if (indexSetParams != 1) sbSetParams.append(", ");

      sbSetParams.append(fiCol.getTxDbFieldNameOrFieldName())
          .append("=")
          .append("@")
    ...
    if(FiString.isEmptyTrim(sbWhereCond.toString())){
...
//        }
   ...
   * @return
   */
  public static String updateQueryWithFiMapParamByCandId(Class clazz, Fkb fiKeyBean) {
...
//				}
  ...
  public static String wrapQueryWithRowConstraint(String txQuery) {
    ...
    return FiString.substitutor(tempWrapQuery, Fkb.bui().buiPut("txQuery", txQuery));
