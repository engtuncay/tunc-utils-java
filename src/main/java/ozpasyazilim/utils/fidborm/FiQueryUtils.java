...
/**
 * Sorgu üzerindeki işlemler için util metodlar
 * <p>
 * Fiqu -> FiQueryUtils kısaltması
 */
public class FiQueryUtils {
  ...
  public static String deActivateOptParamsNotUsed(String txQuery, Fkb mapParams) {
    ...
    for (String optParam : hstParamsOptional) {
      ...
      if (!mapParams.containsKey(optParam)) {
        ...
        spQuery.set(FiQueryUtils.deActivateOptParamMain(spQuery.get(), optParam));
        //mapParams.remove(deActivatedParam);
      }
  ...
  private static String convertSingleParamToMultiParamWithComma(String txQuery, Fkb mapParams, String param, Collection collParamData, Boolean boKeepOldParam) {
    ...
    for (Object listDatum : collParamData) {
      String sablonParam = FiQueryUtils.makeMultiParamTemplate(param, index);
  ...
  public static String getSqlCountQueryFromQuery(String txQuery) {
    ...
    if (matcher.find()) {
      ...
      result = FiQueryUtils.deActivateForSqlCount(result);
      return result;
    } else {
      return null;
    }
